...
| Elapsed working time, whole effort | **about 140 h** over 37 days (Aug 14 to Sep 19), including ~53 h in two approaches that were abandoned |
...
| Point | Unsafe lines | Share of code | `unsafe fn` | real `static mut` | raw-pointer tokens | `c_int`-style C types |
|---|---|---|---|---|---|---|
| c2rust output (Sep 5) | 48,430 | 33 % | ~1,400 | 1,639 | 10,456 | 52,321 |
| Sep 6 evening | 41,557 | 69 % | 1,142 | 890 | 5,543 | 2,905 |
...
## 6. What was done, at a high level

**Before the pivot (Aug 14 to Sep 5, two attempts).** Write our own C parser, then a
...
threaded as `&mut`, over ~96 small "bridge collapse" PRs. 1,639 `static mut` -> 0.
...
- **`static mut`:** the column counts real `static mut` declarations (`git grep`), not
  `&'static mut` types. The per-commit CSV column `static_mut` also matches `&'static mut`, so
