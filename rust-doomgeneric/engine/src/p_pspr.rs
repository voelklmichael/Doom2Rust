...
pub fn bring_up_weapon(state: &mut GameState, player_id: PlayerId) {
    ...
    if state.game.g_game.players[player].pendingweapon == WeaponType::Chainsaw {
...
pub fn check_ammo(state: &mut GameState, player_id: PlayerId) -> bool {
    ...
    let ammo: AmmoType = WEAPONINFO[state.game.g_game.players[player].readyweapon].ammo;
    let count: i32 = if state.game.g_game.players[player].readyweapon == WeaponType::Bfg {
        DEH_BFG_CELLS_PER_SHOT
    } else if state.game.g_game.players[player].readyweapon == WeaponType::Supershotgun {
        2
    } else {
        1
    };
    if ammo == AmmoType::Noammo || state.game.g_game.players[player].ammo[ammo] >= count {
        return true;
    }
    loop {
    ...
    set_psprite(
        ...
        WEAPONINFO[state.game.g_game.players[player].readyweapon].downstate,
    );
    false
}
pub fn fire_weapon(state: &mut GameState, player_id: PlayerId) {
    ...
    let newstate: StateNum = WEAPONINFO[state.game.g_game.players[player].readyweapon].atkstate;
    set_psprite(state, player_id, PSpriteNum::Weapon as i32, newstate);
...
pub fn weapon_ready(state: &mut GameState, player: PlayerId, position: i32) {
    ...
    if state.game.g_game.players[player].readyweapon == WeaponType::Chainsaw
        && state.game.g_game.players[player].psprites[position.idx()].state
    ...
    if state.game.g_game.players[player].cmd.buttons & BT_ATTACK != 0 {
        if !state.game.g_game.players[player].attackdown
            || state.game.g_game.players[player].readyweapon != WeaponType::Missile
                && state.game.g_game.players[player].readyweapon != WeaponType::Bfg
        {
    ...
    state.game.g_game.players[player].psprites[position.idx()].sx = FRACUNIT
        + fixed_mul(
            state.game.g_game.players[player].bob,
            fine_cosine(angle.idx()),
        );
    angle &= FINEANGLES / 2 - 1;
    state.game.g_game.players[player].psprites[position.idx()].sy = 32 * FRACUNIT
        + fixed_mul(
            state.game.g_game.players[player].bob,
            fine_sine(angle.idx()),
        );
...
pub fn lower(state: &mut GameState, player: PlayerId, position: i32) {
    state.game.g_game.players[player].psprites[position.idx()].sy += FRACUNIT * 6;
    if state.game.g_game.players[player].psprites[position.idx()].sy < 128 * FRACUNIT {
        return;
    }
    if state.game.g_game.players[player].playerstate == PlayerState::Dead {
        state.game.g_game.players[player].psprites[position.idx()].sy = 128 * FRACUNIT;
        return;
    }
...
pub fn raise(state: &mut GameState, player: PlayerId, position: i32) {
    state.game.g_game.players[player].psprites[position.idx()].sy -= FRACUNIT * 6;
    if state.game.g_game.players[player].psprites[position.idx()].sy > 32 * FRACUNIT {
        return;
    }
    state.game.g_game.players[player].psprites[position.idx()].sy = 32 * FRACUNIT;
...
pub fn gun_flash(state: &mut GameState, player: PlayerId, _position: i32) {
    ...
    set_psprite(
        ...
        WEAPONINFO[state.game.g_game.players[player].readyweapon].flashstate,
    );
}
...
fn decrease_ammo(g_game: &mut GGameState, player: PlayerId, ammonum: usize, amount: i32) {
...
pub fn fire_plasma(state: &mut GameState, player: PlayerId, _position: i32) {
    ...
    let flashstate = statenum_from_raw(
        WEAPONINFO[state.game.g_game.players[player].readyweapon].flashstate as i32
...
pub fn fire_cgun(state: &mut GameState, player: PlayerId, position: i32) {
    ...
    set_psprite(
        ...
        PSpriteNum::Flash as i32,
        statenum_from_raw(
            (WEAPONINFO[state.game.g_game.players[player].readyweapon].flashstate as i64
                + i64::from(
                    state.game.g_game.players[player].psprites[position.idx()]
                        .state
...
pub fn move_psprites(state: &mut GameState, player_id: PlayerId) {
    for i in 0..NUMPSPRITES {
        let psp_state = state.game.g_game.player_mut(player_id).psprites[i].state;
        if let Some(psp_state) =
            psp_state.filter(|_| state.game.g_game.player_mut(player_id).psprites[i].tics != -1)
        {
            let psp = &mut state.game.g_game.player_mut(player_id).psprites[i];
            psp.tics -= 1;
            if psp.tics == 0 {
                let nextstate = state.assets.info.state_mut(psp_state).nextstate;
                set_psprite(state, player_id, i as i32, nextstate);
            }
        }
    }
