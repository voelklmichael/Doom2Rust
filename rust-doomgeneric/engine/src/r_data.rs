...
pub fn init_textures(state: &mut GameState) {
    ...
    for i in 0..nummappatches {
        ...
        let patch_name = FixedCStr::<8>::from_bytes(&pnames[name_off..name_off + 8])
            .as_str()
            .into_owned();
        patchlookup[i as usize] =
            check_num_for_name(&state.assets.w_wad, &patch_name).unwrap_or(-1);
    }
    ...
    while i < state.render.r_data.numtextures {
        ...
        state.render.r_data.textureheight[i as usize] =
            ((texture_height as i32) << FRACBITS) as Fixed;
        i += 1;
        dir_index += 1;
    }
...
pub fn init_sprite_lumps(state: &mut GameState) {
    ...
    for i in 0..state.render.r_data.numspritelumps {
        ...
        state.render.r_data.spritewidth[i as usize] = ((width as i32) << FRACBITS) as Fixed;
        state.render.r_data.spriteoffset[i as usize] = ((leftoffset as i32) << FRACBITS) as Fixed;
...
pub fn precache_level(state: &mut GameState) {
    ...
    flatpresent = vec![0u8; state.render.r_data.numflats as usize];
    ...
    texturepresent = vec![0u8; state.render.r_data.numtextures as usize];
    ...
    spritepresent = vec![0u8; state.render.r_things.numsprites as usize];
