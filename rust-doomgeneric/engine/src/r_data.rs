...
pub fn draw_column_in_cache(patch: &[u8], cache: &mut [u8], originy: i32, cacheheight: i32) {
    ...
    while i32::from(patch[cursor]) != 0xff {
        ...
        let source = &patch[cursor + 3..cursor + 3 + length.idx()];
        ...
        if count > 0 {
            cache[position.idx()..(position + count).idx()].copy_from_slice(&source[..count.idx()]);
        }
        cursor += length.idx() + 4;
    }
}
pub fn generate_composite(
    ...
    let mut block: Vec<u8> = vec![0u8; r_data.texturecompositesize[texnum.idx()].idx() + 128];
    let texture_patchcount = i32::from(r_data.textures[texnum.idx()].patchcount);
    let texture_width = i32::from(r_data.textures[texnum.idx()].width);
    let texture_height = i32::from(r_data.textures[texnum.idx()].height);
    for i in 0..texture_patchcount.idx() {
        let tex_patch = r_data.textures[texnum.idx()].patches[i];
        ...
        let realpatch_len = lump_length(w_wad, tex_patch.patch).idx();
        let realpatch_lump = lump_bytes(fs, w_wad, tex_patch.patch);
        let realpatch = &realpatch_lump[..realpatch_len];
        let realpatch_width = i32::from(le_i16(realpatch, 0));
        let x1: i32 = i32::from(tex_patch.originx);
        let x2: i32 = (x1 + realpatch_width).min(texture_width);
        for x in x1.max(0)..x2 {
            if i32::from(r_data.texturecolumnlump[texnum.idx()][x.idx()]) < 0 {
                let colofs_off = (8 + (x - x1) * 4).idx();
                let columnofs = le_i32(realpatch, colofs_off);
                let patchcol = &realpatch[columnofs.idx()..];
                let cache_off = r_data.texturecolumnofs[texnum.idx()][x.idx()] as usize;
    ...
    r_data.texturecomposite[texnum.idx()] = Some(block.into_boxed_slice());
}
pub fn generate_lookup(state: &mut GameState, texnum: i32) {
    state.render.r_data.texturecomposite[texnum.idx()] = None;
    state.render.r_data.texturecompositesize[texnum.idx()] = 0;
    let texture_patchcount = i32::from(state.render.r_data.textures[texnum.idx()].patchcount);
    let texture_width = i32::from(state.render.r_data.textures[texnum.idx()].width);
    let texture_height = i32::from(state.render.r_data.textures[texnum.idx()].height);
    let mut patchcount: Vec<u8> = vec![0u8; texture_width.idx()];
    for i in 0..texture_patchcount.idx() {
        let tex_patch = state.render.r_data.textures[texnum.idx()].patches[i];
        let realpatch_len = lump_length(&state.assets.w_wad, tex_patch.patch).idx();
        let realpatch_lump =
        ...
        for x in x1.max(0)..x2 {
            patchcount[x.idx()] = patchcount[x.idx()].wrapping_add(1);
            state.render.r_data.texturecolumnlump[texnum.idx()][x.idx()] = tex_patch.patch.0 as i16;
            let colofs_off = (8 + (x - x1) * 4).idx();
            let columnofs = le_i32(realpatch, colofs_off);
            state.render.r_data.texturecolumnofs[texnum.idx()][x.idx()] =
                (columnofs + 3).cast_unsigned() as u16;
        }
    }
    for (x, &count) in patchcount.iter().enumerate().take(texture_width.idx()) {
        if count == 0 {
            doom_println!(
                state.io.platform,
                "R_GenerateLookup: column without a patch ({})",
                state.render.r_data.textures[texnum.idx()].name.as_str(),
            );
            return;
        }
        if count > 1 {
            state.render.r_data.texturecolumnlump[texnum.idx()][x] = -1_i16;
            state.render.r_data.texturecolumnofs[texnum.idx()][x] =
                state.render.r_data.texturecompositesize[texnum.idx()].cast_unsigned() as u16;
            if state.render.r_data.texturecompositesize[texnum.idx()] > 0x10000 - texture_height {
                error(&format!("R_GenerateLookup: texture {texnum} is >64k"));
            }
            state.render.r_data.texturecompositesize[texnum.idx()] += texture_height;
        }
    }
}
pub fn get_column(
    ...
    col &= r_data.texturewidthmask[tex.idx()];
    let lump = r_data.texturecolumnlump[tex.idx()][col.idx()];
    let ofs: i32 = i32::from(r_data.texturecolumnofs[tex.idx()][col.idx()]);
    if lump > 0 {
        let lump = LumpNum(i32::from(lump).cast_unsigned());
    ...
    if r_data.texturecomposite[tex.idx()].is_none() {
...
fn generate_texture_hash_table(r_data: &mut RDataState) {
    r_data.textures_hashtable = vec![None; r_data.numtextures.idx()];
    for i in 0..r_data.numtextures {
        r_data.textures[i.idx()].index = i;
        r_data.textures[i.idx()].next = None;
        let key: usize = (lump_name_hash(r_data.textures[i.idx()].name.as_bytes())
            .wrapping_rem(r_data.numtextures.cast_unsigned()) as i32)
            .idx();
        ...
        match r_data.textures_hashtable[key] {
            None => {
                r_data.textures_hashtable[key] = Some(TextureId(i.cast_unsigned()));
            }
            Some(mut cursor) => {
                while let Some(next) = r_data.textures[cursor.0 as usize].next {
                    cursor = next;
                }
                r_data.textures[cursor.0 as usize].next = Some(TextureId(i.cast_unsigned()));
            }
        }
    }
}
pub fn init_textures(state: &mut GameState) {
    ...
    let pnames = lump_bytes_name(&*state.assets.fs, &mut state.assets.w_wad, "PNAMES")
        [..pnames_len]
        .to_vec();
    let nummappatches: i32 = le_i32(&pnames, 0);
    let mut patchlookup: Vec<Option<LumpNum>> = vec![None; nummappatches.idx()];
    for i in 0..nummappatches {
        let name_off = 4 + (i * 8).idx();
    ...
    let maptex1 = lump_bytes_name(&*state.assets.fs, &mut state.assets.w_wad, "TEXTURE1")
        [..maxoff.idx()]
        .to_vec();
    let numtextures1: i32 = le_i32(&maptex1, 0);
    let (maptex2, maxoff2, numtextures2) =
        if check_num_for_name(&state.assets.w_wad, "TEXTURE2").is_none() {
            ...
            let maptex2 = lump_bytes_name(&*state.assets.fs, &mut state.assets.w_wad, "TEXTURE2")
                [..maxoff2.idx()]
                .to_vec();
            let numtextures2 = le_i32(&maptex2, 0);
    ...
    for i in 0..state.render.r_data.numtextures {
        ...
        let dir_off = 4 + (dir_index * 4).idx();
        let offset: i32 = le_i32(current_maptex, dir_off);
        if offset > maxoff {
        ...
        state
            ...
            .r_data
            .texturecolumnlump
            .push(vec![0i16; texture_width.idx()]);
        state
            .render
            .r_data
            .texturecolumnofs
            .push(vec![0u16; texture_width.idx()]);
        let mut j: i32 = 1;
        while j * 2 <= i32::from(texture_width) {
            j <<= 1;
        }
        state.render.r_data.texturewidthmask[i.idx()] = j - 1;
        state.render.r_data.textureheight[i.idx()] = Fixed::from_int(i32::from(texture_height));
        dir_index += 1;
    }
    ...
    state.render.r_data.texturetranslation = vec![0; (state.render.r_data.numtextures + 1).idx()];
    for i in 0..state.render.r_data.numtextures {
        state.render.r_data.texturetranslation[i.idx()] = i;
    }
...
pub fn init_flats(r_data: &mut RDataState, w_wad: &WWadState) {
    ...
    r_data.flattranslation = vec![0; (r_data.numflats + 1).idx()];
    for i in 0..r_data.numflats {
        r_data.flattranslation[i.idx()] = i;
    }
}
pub fn init_sprite_lumps(state: &mut GameState) {
    ...
    state.render.r_data.spritewidth = vec![Fixed::ZERO; state.render.r_data.numspritelumps.idx()];
    state.render.r_data.spriteoffset = vec![Fixed::ZERO; state.render.r_data.numspritelumps.idx()];
    state.render.r_data.spritetopoffset =
        vec![Fixed::ZERO; state.render.r_data.numspritelumps.idx()];
    for i in 0..state.render.r_data.numspritelumps {
        ...
        let header = &lump[..8];
        let width = le_i16(header, 0);
        let leftoffset = le_i16(header, 4);
        let topoffset = le_i16(header, 6);
        state.render.r_data.spritewidth[i.idx()] = Fixed::from_int(i32::from(width));
        state.render.r_data.spriteoffset[i.idx()] = Fixed::from_int(i32::from(leftoffset));
        state.render.r_data.spritetopoffset[i.idx()] = Fixed::from_int(i32::from(topoffset));
    }
}
pub fn init_colormaps(fs: &dyn DoomFileSystem, r_data: &mut RDataState, w_wad: &mut WWadState) {
    ...
    let lumplen = lump_length(w_wad, lump).idx();
...
pub fn check_texture_num_for_name(state: &RDataState, name: &str) -> Option<i32> {
    ...
    let key: i32 =
        lump_name_hash(name.as_bytes()).wrapping_rem(state.numtextures.cast_unsigned()) as i32;
    let mut cursor = state.textures_hashtable[key.idx()];
...
pub fn precache_level(state: &mut GameState) {
    ...
    let mut flatpresent: Vec<u8> = vec![0u8; state.render.r_data.numflats.idx()];
    for i in 0..state.world.p_setup.numsectors.idx() {
        flatpresent[state.world.p_setup.sectors[i].floorpic.idx()] = 1;
        flatpresent[state.world.p_setup.sectors[i].ceilingpic.idx()] = 1;
    }
    state.render.r_data.flatmemory = 0;
    for i in 0..state.render.r_data.numflats {
        if flatpresent[i.idx()] != 0 {
    ...
    let mut texturepresent: Vec<u8> = vec![0u8; state.render.r_data.numtextures.idx()];
    for i in 0..state.world.p_setup.numsides.idx() {
        texturepresent[state.world.p_setup.sides[i].toptexture.idx()] = 1;
        texturepresent[state.world.p_setup.sides[i].midtexture.idx()] = 1;
        texturepresent[state.world.p_setup.sides[i].bottomtexture.idx()] = 1;
    }
    texturepresent[state.render.r_sky.skytexture.idx()] = 1;
    ...
    let mut spritepresent: Vec<u8> = vec![0u8; state.render.r_things.numsprites.idx()];
    ...
    for (i, &present) in spritepresent.iter().enumerate() {
        if present != 0 {
            for j in 0..state.render.r_things.sprites[i].numframes.idx() {
                for k in 0..8 {
