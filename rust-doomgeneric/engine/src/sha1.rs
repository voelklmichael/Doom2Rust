...
pub unsafe fn SHA1_Final(mut digest: *mut byte, mut hd: *mut sha1_context_t) {
    ...
    (*hd).buf[56] = (msb >> 24 as i32) as byte;
    (*hd).buf[57] = (msb >> 16 as i32) as byte;
    (*hd).buf[58] = (msb >> 8 as i32) as byte;
    (*hd).buf[59] = msb as byte;
    (*hd).buf[60] = (lsb >> 24 as i32) as byte;
    (*hd).buf[61] = (lsb >> 16 as i32) as byte;
    (*hd).buf[62] = (lsb >> 8 as i32) as byte;
...
pub unsafe fn SHA1_UpdateInt32(mut context: *mut sha1_context_t, mut val: u32) {
    ...
    buf[0] = (val >> 24 as i32 & 0xff as u32) as byte;
    buf[1] = (val >> 16 as i32 & 0xff as u32) as byte;
    buf[2] = (val >> 8 as i32 & 0xff as u32) as byte;
    buf[3] = (val & 0xff as u32) as byte;
